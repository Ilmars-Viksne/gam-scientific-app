...
from .io_utils import sha256_file, stable_hash, utc_now, write_json_atomic, write_yaml_atomic
...
def create_run(config_path: Path, workspace: Path) -> Path:
    ...
    run_id = f"run-{utc_now().replace(':', '').replace('+00:00', 'Z')}-{uuid.uuid4().hex[:8]}"
    ...
    write_json_atomic(store.root / "run.json", {
        "schema_version": "1.0",
        "run_id": run_id,
    ...
    write_json_atomic(store.root / "environment.json", {
        "python": sys.version,
...
def execute_run(run_directory: Path) -> None:
    ...
    try:
        ...
        write_json_atomic(run_directory / "data_manifest.json", {
        ...
        store.update_status(state="running", phase="evaluation", started_at_utc=utc_now())
        store.event("run_started")
        for model in config.models:
            run_model(config, model, X, y, row_ids, splits, store, run["data_hash"], run["config_hash"])
            if store.requested("PAUSE") or store.requested("CANCEL"):
                return
            fit_final_model(config, model, X, y, store)
        create_reports(config, store)
        store.update_status(state="completed", phase="reporting", completed_at_utc=utc_now())
